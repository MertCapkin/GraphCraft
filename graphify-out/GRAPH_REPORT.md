...
## Corpus Check
- 32 files · ~11,666 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 610 nodes · 849 edges · 33 communities (28 shown, 5 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 81 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b7494691`
...
1. `echo()` - 23 edges
2. `run_checks()` - 22 edges
3. `GraphStack 🧠⚡` - 18 edges
4. `compact_command_output()` - 13 edges
5. `run_git()` - 12 edges
6. `ORCHESTRATOR` - 12 edges
7. `Transition Rules` - 12 edges
8. `_feed_stdin()` - 11 edges
9. `Token Optimizer` - 11 edges
10. `hook_cursor()` - 10 edges

## Surprising Connections (you probably didn't know these)
- `cmd_clear()` --calls--> `echo()`  [INFERRED]
  scripts/graphstack/state.py → scripts/graphstack/platform_utils.py
...
Cohesion: 0.06
Nodes (66): _build_parser(), cmd_claim(), cmd_complete(), cmd_log(), cmd_new(), cmd_status(), _get(), _git_commit_board() (+58 more)

### Community 1 - "Community 1"
Cohesion: 0.08
Nodes (47): git_available(), _brief_is_template(), _brief_status(), _build_parser(), check_board_tasks(), check_brief(), check_compact_module(), check_framework_handoff() (+39 more)

### Community 2 - "Community 2"
Cohesion: 0.05
Nodes (44): All Prompts, Bootstrap Mode — Start from Zero, code:block12 (You:  "Add rate limiting to login."), code:block13 (Step 1 — once:), code:block14 (Composer (Cursor):), code:block15 (your-project/), code:bash (bash scripts/board.sh status), code:powershell (.\scripts\board.ps1 status) (+36 more)
...
Cohesion: 0.09
Nodes (20): _feed_stdin(), _hook_output(), _make_doing_task(), Tests for the deterministic process gate (rules, both hook adapters, bypass)., test_claude_bash_hook_denies_commit(), test_claude_edit_hook_allows_with_task(), test_claude_edit_hook_denies_with_wrapper(), test_claude_hook_fails_open_on_garbage_stdin() (+12 more)

### Community 6 - "Community 6"
Cohesion: 0.11
Nodes (33): Path and configuration constants used across the package.  Paths are resolved, _brief_is_unwritten(), _changed_files(), _commit_candidate_files(), _doing_tasks(), _emit(), evaluate_command(), evaluate_file_edit() (+25 more)

### Community 7 - "Community 7"
Cohesion: 0.1
Nodes (30): CompactResult, dedupe_consecutive(), is_critical_line(), Shared helpers for safe output compaction., Keep critical lines and a head/tail window; return (lines, omitted_count)., Return compacted text unless it lost too much signal vs raw., safe_compact(), truncate_preserving_critical() (+22 more)
...
Cohesion: 0.18
Nodes (9): _build_parser(), main(), Top-level CLI dispatcher.  Eight sub-commands: - ``board``     — GNAP task bo, Entry point for both ``python -m graphstack`` and unit tests., Entry point for both ``python -m graphstack`` and unit tests., Entry point for both ``python -m graphstack`` and unit tests., Entry point for both ``python -m graphstack`` and unit tests., GraphStack — cross-platform Python core.  This package replaces the bash/Power (+1 more)
...
Cohesion: 0.43
Nodes (7): _build_parser(), execute(), _quality_git_argv(), Run shell commands with token-safe output compaction.  Usage:   graphstack ru, Use porcelain status for reliable path preservation when compacting., run(), _strip_leading_dashes()
...
## Knowledge Gaps
- **233 isolated node(s):** `GNAP board manager — pure Python port of ``scripts/board.sh``.  JSON schema is`, `Stage the board directory and commit silently — never fails the command.`, `Top-level CLI dispatcher.  Eight sub-commands: - ``board``     — GNAP task bo`, `Entry point for both ``python -m graphstack`` and unit tests.`, `Path and configuration constants used across the package.  Paths are resolved` (+228 more)
...
- **Why does `echo()` connect `Community 0` to `Community 1`, `Community 6`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 16` to `Community 1`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Are the 20 inferred relationships involving `echo()` (e.g. with `_print_task()` and `cmd_status()`) actually correct?**
  _`echo()` has 20 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 10 inferred relationships involving `compact_command_output()` (e.g. with `execute()` and `CompactResult`) actually correct?**
  _`compact_command_output()` has 10 INFERRED edges - model-reasoned connections that need verification._
- **Are the 10 inferred relationships involving `run_git()` (e.g. with `_git_commit_board()` and `cmd_log()`) actually correct?**
  _`run_git()` has 10 INFERRED edges - model-reasoned connections that need verification._
- **What connects `GNAP board manager — pure Python port of ``scripts/board.sh``.  JSON schema is`, `Stage the board directory and commit silently — never fails the command.`, `Top-level CLI dispatcher.  Eight sub-commands: - ``board``     — GNAP task bo` to the rest of the system?**
