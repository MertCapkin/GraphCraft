...
def run(argv: list[str]) -> int:
    ...
    rule_file = target / ".cursor" / "rules" / "graphstack.mdc"
    if not rule_file.is_file():
        echo("Error: install finished but .cursor/rules/graphstack.mdc is missing.")
        echo('  pip install -U --force-reinstall "MertCapkin_GraphStack[graphify]"')
        echo("  graphstack init . -y --install-deps")
        return 1

    if doctor_rc != 0:
        echo("Note: doctor reported issues above; core GraphStack files are installed.")
        return 0
